...
def save_metrics_summary(
    ...
    for metric, scores in per_metric_scores.items():
        ...
        stdev = math.sqrt(sum((x - avg) ** 2 for x in scores) / (len(scores) - 1)) if len(scores) > 1 else 0.0
        formatted = f"{avg:.2f} ± {stdev:.2f}" if len(scores) > 1 else f"{avg:.2f}"

        if any(metric.startswith(f"{k}.") for k in DETERMINISTIC_METRICS):
            det_summary[metric] = {"average": avg, "stdev": stdev, "formatted": formatted}
        elif metric in DETERMINISTIC_METRICS:
            continue
        else:
            # Include score_range if available
            metric_data = {"average": avg, "stdev": stdev, "formatted_mean_stdev": formatted, "runs": len(scores)}
