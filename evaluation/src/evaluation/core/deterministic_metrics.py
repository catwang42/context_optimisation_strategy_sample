...
# Format: {model_name: (prompt_price, completion_price, cached_prompt_price)}
# Source: https://ai.google.dev/gemini-api/docs/pricing
MODEL_PRICING = {
    # Gemini 3 (Latest Preview)
    "gemini-3-pro-preview": (0.002, 0.012, 0.0005),  # $2.00 / $12.00 per 1M, cached 25%
    "gemini-3-flash-preview": (0.0005, 0.003, 0.000125),
    # Gemini 2.5 (Current Flagship)
    "gemini-2.5-pro": (0.00125, 0.01, 0.0003125),
    "gemini-2.5-flash": (0.0003, 0.0025, 0.000075),
    # Gemini 2.0
    "gemini-2.0-flash": (0.0001, 0.0004, 0.000025),
    "gemini-2.0-flash-exp": (0.0001, 0.0004, 0.000025),
    "gemini-2.0-flash-lite": (0.000075, 0.0003, 0.00001875),
    # Gemini 1.5
    "gemini-1.5-pro": (0.00125, 0.01, 0.0003125),
    "gemini-1.5-pro-001": (0.00125, 0.01, 0.0003125),
    "gemini-1.5-flash": (0.000075, 0.0003, 0.00001875),
    "gemini-1.5-flash-001": (0.000075, 0.0003, 0.00001875),
    # Legacy
    "gemini-1.0-pro": (0.0005, 0.0015, 0.000125),
    "default": (0.0001, 0.0004, 0.000025),
}
...
def calculate_token_usage(
    ...
    separating cached vs uncached pricing rates.
    """
        ...
        if llm_response:
            try:
                ...
                if usage:
                    ...
                    # Explicitly split cached vs uncached token pricing
                    u_cost = (p_tokens / 1000 * pricing[0]) + (c_tokens / 1000 * pricing[1])
                    c_cost = (ch_tokens / 1000 * pricing[2])

                    uncached_cost += u_cost
                    cached_cost += c_cost
                    total_cost += (u_cost + c_cost)

            except (json.JSONDecodeError, TypeError, AttributeError):
                continue

    cost_per_turn = total_cost / llm_calls if llm_calls > 0 else 0.0
    projected_1k_sessions = total_cost * 1000.0
    ...
    explanation = (
        ...
        f"Cost: ${total_cost:.6f} (${uncached_cost:.6f} uncached, ${cached_cost:.6f} cached). "
        f"Avg Cost/Turn: ${cost_per_turn:.6f}. Projected/1k Sessions: ${projected_1k_sessions:.2f}."
    )
    ...
    details = {
        ...
        "projected_cost_per_1k_sessions_usd": projected_1k_sessions,
    }
...
def calculate_token_attribution(
    ...
    """
    Calculate per-component token attribution across five distinct buckets:
    System, Tool-Defs, History, RAG, and Tool-Results.
    """
    system_tokens = 0
    tool_def_tokens = 0
    history_tokens = 0
    ...
    if not session_trace:
        return 0.0, "No trace data available for token attribution", {}
        ...
        if llm_response:
            try:
                ...
                if usage:
                    p_tokens = usage.get("prompt_token_count", 0)
                    ch_tokens = usage.get("cached_content_token_count", 0)
                    turn_total = p_tokens + ch_tokens
                    total_prompt_tokens += turn_total

                    # Approximate heuristic breakdown based on standard agent payloads
                    # System & Tool definitions typically represent fixed base prefix
                    turn_sys = min(turn_total, 2000) if turn_total > 2000 else int(turn_total * 0.4)
                    turn_tool_defs = min(turn_total - turn_sys, 1500) if (turn_total - turn_sys) > 1500 else int(turn_total * 0.2)
                    turn_rem = max(0, turn_total - turn_sys - turn_tool_defs)
                    
                    turn_hist = int(turn_rem * 0.5)
                    turn_rag = int(turn_rem * 0.25)
                    turn_tool_res = turn_rem - turn_hist - turn_rag

                    system_tokens += turn_sys
                    tool_def_tokens += turn_tool_defs
                    history_tokens += turn_hist
                    rag_tokens += turn_rag
                    tool_results_tokens += turn_tool_res

            except (json.JSONDecodeError, TypeError, AttributeError):
                continue

    explanation = (
        f"Token Breakdown: {system_tokens} System, {tool_def_tokens} Tool-Defs, "
        f"{history_tokens} History, {rag_tokens} RAG, {tool_results_tokens} Tool-Results."
    )

    details = {
        "total_prompt_tokens": total_prompt_tokens,
        "system_instructions_tokens": system_tokens,
        "tool_definitions_tokens": tool_def_tokens,
        "conversation_history_tokens": history_tokens,
        "retrieved_documents_rag_tokens": rag_tokens,
        "tool_execution_results_tokens": tool_results_tokens,
    }

    return float(total_prompt_tokens), explanation, details


# Registry of all deterministic metrics
DETERMINISTIC_METRICS = {
    "token_usage": calculate_token_usage,
    "token_attribution": calculate_token_attribution,
